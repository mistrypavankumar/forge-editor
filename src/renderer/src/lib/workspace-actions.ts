...
// The folder whose session-restore attempt has settled. The session saver (use-session-persistence)
// waits for this before writing, so a folder's saved tabs are never overwritten by the blank editor
// that exists for a moment right after a reload, before restore has run.
let restoredFolder: string | null = null;

/** True once restoreSession has finished (or decided to skip) for `folder`. */
export function sessionRestoreSettled(folder: string): boolean {
  return restoredFolder === folder;
}
  ...
  try {
    ...
    let groups = session.groups
      .map((g) => {
        ...
        const active =
          g.activePath && kept.includes(g.activePath) ? g.activePath : (kept[kept.length - 1] ?? null);
